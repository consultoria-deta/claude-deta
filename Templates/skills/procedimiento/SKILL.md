...
- El campo `titulo` de cada subsección del procedimiento **nunca debe incluir el prefijo numérico** (ej. `"7.1"`, `"7.2"`). La función `_section_procedimiento` del template los agrega automáticamente al renderizar. Si se incluyen en el dict, el número aparece duplicado en el documento final (`7.1 7.1 Prospección`).
...
datos = {
    ...
    "procedimiento": [                          # list[dict]
        {
            "titulo": "Nombre del paso o subsección",  # ⚠️ SIN prefijo numérico — el template agrega 7.1, 7.2… automáticamente
            "pasos": [                          # list[str | dict]
                "Rol → Acción → Resultado",
                {                              # sub-subsección opcional
                    "titulo": "A. Subtipo",    # ⚠️ SIN prefijo numérico aquí tampoco
