...
```
firecrawl scrape [sitio relevante del paso 2]   # preferido — maneja JS, Wikipedia, anti-bot básico
...
> **Herramienta de scraping:** usar `firecrawl scrape <url>` (instancia local en `http://localhost:3002`, sin costo). `web_fetch` solo como fallback para URLs simples que no requieran JS. Para agregar el contenido a NotebookLM: si la URL falla en `notebooklm source add`, guardar el markdown con `firecrawl scrape <url> > /tmp/fuente.md` y agregar como archivo.
